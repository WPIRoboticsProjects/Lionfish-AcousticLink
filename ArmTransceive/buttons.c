...
volatile DataType buttons_fifo[FIFO_SIZE];  // FIFO storage array
extern volatile int fifo_head = 0; // index of the first item in the FIFO
extern volatile int fifo_tail = 0; // index one step past the last item
...
void ButtonISR(void) {
    ...
    uint32_t gpio_buttons =
            ...
            (~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & (GPIO_PIN_1))<<1; // EK-TM4C1294XL buttons in positions 0 and 1

    // put buttons in fifo
//    fifo_put((char) gpio_buttons);
...
int fifo_put(DataType data)
{
    ...
    if (fifo_head != new_tail) {    // if the FIFO is not full
        buttons_fifo[fifo_tail] = data;     // store data into the FIFO
...
int fifo_get(DataType *data)
{
    if (fifo_head != fifo_tail) {   // if the FIFO is not empty
        *data = buttons_fifo[fifo_head];    // read data from the FIFO
